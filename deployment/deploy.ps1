...
.SYNOPSIS
    ...
    deploy.ps1 - Deployment script for teams-webchat-sample
    https://github.com/leeford/teams-webchat-sample
 
.DESCRIPTION
    Author: Lee Ford

    This tool allows you to deploy teams-webchat-sample. See https://www.github.com/leeford/teams-webchat-sample for more details.
...
Write-Host "`n----------------------------------------------------------------------------------------------
            `n deploy.ps1 - Lee Ford - https://github.com/leeford/teams-webchat-sample
...
if ((Test-Path $templateFile) -and (Test-Path $parametersFile)) {
    ...
    try {
        ...
        $deploy = New-AzResourceGroupDeployment -Name "teams_webchat_deployment_powershell_$timeStamp" -ResourceGroupName $ResourceGroup -TemplateFile $templateFile -TemplateParameterFile $parametersFile -ErrorAction Stop
    ...
    try {
        ...
        if (Test-Path $manifestTemplatePath) {
            ...
            $teamsAppManifestContent = $teamsAppManifestContent.Replace("<Bot ID>", $parametersObj.parameters.appClientId.Value)
            # Update file
